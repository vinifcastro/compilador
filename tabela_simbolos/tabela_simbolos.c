...
// ... (todo o código até inserir_funcao permanece o mesmo) ...

// O seu código existente aqui...
...
// CORREÇÃO: Função atualizada para aceitar e armazenar os tipos dos parâmetros
void inserir_funcao(TabelaSimbolos* tabela, const char* nome, TipoDado tipo_retorno, int numero_parametros, TipoDado* tipos) {
    if (tabela->qtde_funcoes < MAX_FUNCS) {
        ...
        // Copia os tipos dos parâmetros para a entrada da tabela
...
// ... (código de busca e de pilha permanece o mesmo) ...
...
void pilha_empilhar(PilhaTabelas* p, TabelaSimbolos* tabela) { if (p->topo < MAX_PILHA - 1) p->pilha[++(p->topo)] = tabela; }
void pilha_desempilhar(PilhaTabelas* p) { if (p->topo >= 0) p->topo--; }
TabelaSimbolos* pilha_topo(PilhaTabelas* p) { return (p->topo >= 0) ? p->pilha[p->topo] : NULL; }
...
void iniciar_pilha_tabela() { pilha_iniciar(&pilha_global); criar_escopo(); }
void eliminar_pilha_tabela() { while (pilha_global.topo >= 0) remover_escopo(); }
void criar_escopo() {
...
// CORREÇÃO: Função de wrapper atualizada
...
EntradaTabela* buscar_simbolo(char* nome) { return pilha_buscar_variavel(&pilha_global, nome); }
EntradaFuncao* buscar_funcao_em_escopo(char* nome) { return pilha_buscar_funcao(&pilha_global, nome); }