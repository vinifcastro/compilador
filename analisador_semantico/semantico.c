...
void processar_lista_ids_declaracao(ASTNode *lista_node, TipoDado tipo_herdado) {
    if (!lista_node) return;
    if (lista_node->kind == NODE_ID) {
        char *nome = lista_node->valor_str;
        if (nome && buscar_variavel_no_escopo_atual(nome) == NULL) {
            inserir_variavel(nome, tipo_herdado, 0);
            lista_node->tipo_dado = tipo_herdado;
        } else if (nome) {
...
ASTNode *extrair_parametros_node(ASTNode *func_node) {
    if (func_node && func_node->n_filhos > 0 && func_node->filhos[0] && func_node->filhos[0]->kind == NODE_FUNCAO) {
        ASTNode *inner_func = func_node->filhos[0];
        if (inner_func->n_filhos > 0 && inner_func->filhos[0] && inner_func->filhos[0]->kind == NODE_LISTA) {
            return inner_func->filhos[0];
        }
    }
    return NULL;
}

void inserir_parametros_formais(ASTNode *params_node) {
    if (!params_node || params_node->kind != NODE_LISTA) return;
    for (int i = 0; i < params_node->n_filhos; i++) {
        ASTNode *param = params_node->filhos[i];
        if (param && param->kind == NODE_ID && param->valor_str) {
            inserir_variavel(param->valor_str, param->tipo_dado, 1);
...
TipoDado percorrer_ast_semantica(ASTNode *raiz) {
    ...
    switch (raiz->kind) {
        ...
        case NODE_DECLVAR:
            if (raiz->valor_str) {
                if (buscar_variavel_no_escopo_atual(raiz->valor_str) != NULL) {
                    printf("ERRO SEMÂNTICO: Variável '%s' já declarada (linha %d)\n", raiz->valor_str, raiz->linha);
                    exit(1);
                }
                inserir_variavel(raiz->valor_str, raiz->tipo_dado, 0);
            }
            for (int i = 0; i < raiz->n_filhos; i++) {
                if(raiz->filhos[i]) processar_lista_ids_declaracao(raiz->filhos[i], raiz->tipo_dado);
            }
            break;
        case NODE_FUNCAO:
            if (raiz->valor_str) {
                ...
                inserir_parametros_formais(extrair_parametros_node(raiz));
                for (int i = 0; i < raiz->n_filhos; i++) {
                    if (raiz->filhos[i]) percorrer_ast_semantica(raiz->filhos[i]);
                }
                remover_escopo();
            ...
            break;
        case NODE_ID:
            if (!raiz->valor_str) return TIPO_UNKNOWN;
            EntradaTabela* var = buscar_simbolo(raiz->valor_str);
            if (var) {
                tipo_resultante = var->tipo;
            } else {
                EntradaFuncao* func = buscar_funcao_em_escopo(raiz->valor_str);
                if (func) {
                    tipo_resultante = func->tipo_retorno;
                } else {
                    printf("ERRO SEMÂNTICO: '%s' não foi declarado (linha %d)\n", raiz->valor_str, raiz->linha);
                    exit(1);
                }
            }
        ...
        case NODE_CMD:
            ...
            if (strcmp(raiz->valor_str, "retorne") == 0) {
                TipoDado tr = TIPO_VOID;
                if (raiz->n_filhos > 0 && raiz->filhos[0]) {
                    tr = percorrer_ast_semantica(raiz->filhos[0]);
                }
                if (tipo_retorno_funcao_atual != TIPO_UNKNOWN && tr != tipo_retorno_funcao_atual) {
                    printf("ERRO SEMÂNTICO: Retorno incompatível na função '%s' (linha %d)\n", nome_funcao_atual, raiz->linha);
                    exit(1);
                }
            } else {
                for(int i=0; i < raiz->n_filhos; ++i) {
