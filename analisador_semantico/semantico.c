...
int contar_argumentos(ASTNode* no) {
    ...
    if (no->kind == NODE_ID || no->kind == NODE_CONST || no->kind == NODE_EXPR) return 1;
    return 0;
}

ASTNode* extrair_parametros_node(ASTNode* func_node) {
    if (func_node && func_node->kind == NODE_FUNCAO) {
        if (func_node->n_filhos > 0 && func_node->filhos[0] && func_node->filhos[0]->kind == NODE_LISTA) {
            return func_node->filhos[0]; 
        }
    }
    return NULL;
}
...
TipoDado percorrer_ast_semantica(ASTNode* raiz) {
    ...
    switch (raiz->kind) {
        case NODE_PROGRAMA:
            ...
            for (int i = 0; i < raiz->n_filhos; i++) {
                ...
                if (child->kind == NODE_FUNCAO) {
                    ...
                    // Processa variáveis globais de forma similar a como NODE_DECLVAR é tratado abaixo.
                    // Isso garante que todas as variáveis globais sejam registradas antes que o bloco 'programa' seja analisado.
                    ASTNode* first_id_node = child->filhos[0];
                    if (first_id_node && first_id_node->kind == NODE_ID) {
                        char* nome = first_id_node->valor_str;
                        TipoDado declared_type = child->tipo_dado; // Tipo do DECLVAR (herdado do Tipo)

                        EntradaTabela* entrada_existente = buscar_variavel_no_escopo_atual(nome);
                        if (entrada_existente != NULL) {
                            printf("ERRO SEMÂNTICO: Variável '%s' já declarada neste escopo global (linha %d)\n", nome, child->linha);
                            exit(1);
                        }
                        inserir_variavel(nome, declared_type, 0, 0);
                        first_id_node->tipo_dado = declared_type; // Atualiza o tipo do nó AST para o ID
                    }
                    // Processar a lista de IDs adicionais, se houver
                    if (child->n_filhos > 1 && child->filhos[1]) {
                        processar_lista_ids_declaracao(child->filhos[1], child->tipo_dado);
                    }
                }
            }
        ...
        case NODE_ID: {
            ...
            if (entrada) raiz->tipo_dado = entrada->tipo;
            else if (func) raiz->tipo_dado = func->tipo_retorno; 
            else raiz->tipo_dado = TIPO_UNKNOWN;
