...
// --- NOVAS FUNÇÕES AUXILIARES PARA CHECAGEM DE TIPOS ---

// Coleta os tipos dos parâmetros de uma lista de declaração
...
// Verifica se os tipos dos argumentos de uma chamada batem com os esperados
void verificar_tipos_argumentos(ASTNode* arg_node, EntradaFuncao* func_info, int arg_idx) {
    ...
    if (arg_node->kind == NODE_LISTA) {
        for (int i = 0; i < arg_node->n_filhos; i++) {
            ...
            // Este é um truque para contar o índice corretamente em uma AST plana
            if(arg_node->filhos[i]->kind != NODE_LISTA) arg_idx++;
        }
    } else { // Nó de expressão, constante ou ID
...
// ... (o resto do seu semantico.c até o case NODE_PROGRAMA) ...
// Cole o resto do seu código aqui, e então substitua os cases PROGRAMA e EXPR.
// A seguir estão as versões corrigidas desses dois cases.
...
TipoDado percorrer_ast_semantica(ASTNode *raiz) {
    ...
    switch (raiz->kind) {
        case NODE_PROGRAMA:
            // Passagem 1: Registrar funções e seus parâmetros
            for (int i = 0; i < raiz->n_filhos; i++) {
                ...
                if (child && child->kind == NODE_FUNCAO && child->valor_str) {
                    ...
                    // CORREÇÃO: Coleta os tipos dos parâmetros
        ...
        // ... (Cases BLOCO, DECLVAR, FUNCAO, ID, CONST, CMD... permanecem os mesmos da versão anterior)
        ...
        case NODE_EXPR:
            ...
            // CORREÇÃO: Lógica de chamada de função agora verifica os tipos.
            if (strcmp(raiz->valor_str, "call") == 0) {
                ...
                tipo_resultante = tipo_lhs;
                 ...
                 tipo_resultante = TIPO_INT; // Simplificação: a maioria das expressões resulta em INT
            }
